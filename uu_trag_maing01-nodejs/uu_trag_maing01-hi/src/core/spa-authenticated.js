...
import TragProvider from "./trag-provider.js";
//@@viewOff:imports
...
const SpaAuthenticated = UU5.Common.VisualComponent.create({
  ...
  _getChild(data) {
    return (
      <TragProvider ref={comp => {this._provider = comp}} data={this._buildAppDataContext(data)}>
      ...
      <UU5.Common.Loader onLoad={this._handleLoad}>
        {({ isLoading, isError, data }) => {
          if (isError) {
            ...
            child = <Plus4U5.App.SpaLoading {...this.getMainPropsToPass()}>uuTrag</Plus4U5.App.SpaLoading>;
          } else {
