...
const Errors = require("../api/errors/trag-instance-error");
...
const logger = LoggerFactory.get("UuTrag.Models.TragInstanceModel");

const DEFAULT_NAME = "uuTrag";
...
class TragInstanceAbl {
  constructor() {
    this.validator = new Validator(Path.join(__dirname, "..", "api", "validation_types", "trag-instance-types.js"));
    this.dao = DaoFactory.getDao("tragInstance");
  ...
  async init(uri, dtoIn, session) {
    ...
    // hds 1
    let tragInstance = await this.dao.getByAwid(awid);
    // A1
    if (tragInstance) {
      throw new Errors.Init.TragInstanceAlreadyInitialized();
    }

    // hds 2
    let validationResult = this.validator.validate("tragInstanceInitDtoInType", dtoIn);
    // A2, A3
    ...
    await Promise.all([
      ...
      DaoFactory.getDao("tripRating").createSchema(),
    ...
    // hds 4
    try {
      tragInstance = await this.dao.create(dtoIn);
    } catch (e) {
      // A4
      if (e instanceof ObjectStoreError) {
        throw new Errors.Init.TragInstanceDaoCreateFailed({ uuAppErrorMap }, e);
      }
      throw e;
    }
    ...
    // hds 8
    tragInstance.uuAppErrorMap = uuAppErrorMap;
    return tragInstance;
  }
  ...
  async plugInBt(uri, dtoIn, session) {
    ...
    // A1
    if (!tragInstance) {
      throw new Errors.Load.TragInstanceDoesNotExist();
    }

    // hds 2
    let validationResult = this.validator.validate("tragInstancePlugInBtDtoInType", dtoIn);
    // A2, A3
    let uuAppErrorMap = ValidationHelper.processValidationResult(
      dtoIn,
      validationResult,
  ...
  async load(awid, authorizationResult) {
    // hds 1, A1, hds 1.1, A2
    let tragInstance = await this.checkInstance(
      awid,
      Errors.Load.TragInstanceDoesNotExist,
      Errors.Load.TragInstanceNotInProperState
    );
    // A3
    let authorizedProfiles = authorizationResult.getAuthorizedProfiles();
    if (
      tragInstance.state === STATE_UNDER_CONSTRUCTION &&
      !authorizedProfiles.includes(AUTHORITIES) &&
      !authorizedProfiles.includes(EXECUTIVES)
    ) {
      throw new Errors.Load.TragInstanceIsUnderConstruction({}, { state: tragInstance.state });
    }

    // hds 2
    tragInstance.participantList = (await this.participantDao.list(awid)).itemList;
    tragInstance.locationList = (await this.locationDao.list(awid)).itemList;

    // hds 3
    tragInstance.authorizedProfileList = authorizedProfiles;
  ...
  async update(awid, dtoIn) {
    // hds 1
    let validationResult = this.validator.validate("tragInstanceUpdateDtoInType", dtoIn);
    // A1, A2
    let uuAppErrorMap = ValidationHelper.processValidationResult(
      dtoIn,
      validationResult,
    ...
    try {
      dtoIn.awid = awid;
      tragInstance = await this.dao.updateByAwid(dtoIn);
    } catch (e) {
      if (e instanceof ObjectStoreError) {
        // A6
        throw new Errors.Update.TragInstanceDaoUpdateByAwidFailed({ uuAppErrorMap }, e);
      }
      throw e;
    }
  ...
  async setLogo(awid, dtoIn) {
    // hds 1
    let validationResult = this.validator.validate("tragInstanceSetLogoDtoInType", dtoIn);
    // A1, A2
    let uuAppErrorMap = ValidationHelper.processValidationResult(
      dtoIn,
      validationResult,
    ...
    let tragInstance = await this.checkInstance(
      awid,
      Errors.SetLogo.TragInstanceDoesNotExist,
      Errors.SetLogo.TragInstanceNotInProperState
    );

    // hds 3
    let type = dtoIn.type ? dtoIn.type : DEFAULTS.logoType;
    let binary;
    if (!tragInstance.logos || !tragInstance.logos.includes(type)) {
      // hds 3.1
      try {
    ...
    // hds 4
    if (!tragInstance.logos) tragInstance.logos = [];
    tragInstance.logos.push(type);
    tragInstance.awid = awid;

    try {
      tragInstance = await this.dao.updateByAwid(tragInstance);
    } catch (e) {
      if (e instanceof ObjectStoreError) {
        // A7
        throw new Errors.SetLogo.TragInstanceDaoUpdateByAwidFailed({ uuAppErrorMap }, e);
      }
      throw e;
    }
  ...
  async setIcons(awid, dtoIn, uri) {
    //HDS 1
    let validationResult = this.validator.validate("tragInstanceSetIconsDtoInType", dtoIn);
    // A1, A2
    let uuAppErrorMap = ValidationHelper.processValidationResult(
      dtoIn,
      validationResult,
    ...
    let tragInstance = await this.checkInstance(
      awid,
      Errors.SetIcons.TragInstanceDoesNotExist,
      Errors.SetIcons.TragInstanceNotInProperState
    );

    let uveMetaData = tragInstance.uveMetaData || {};
    let name = tragInstance.name || DEFAULT_NAME;
    ...
    tragInstance.uveMetaData = uveMetaData;

    //HDS 4 A8
    try {
      tragInstance = await this.dao.updateByAwid(tragInstance);
    } catch (e) {
      throw new Errors.SetIcons.TragInstanceDaoUpdateByAwidFailed({ uuAppErrorMap }, e);
    }
    tragInstance.uuAppErrorMap = uuAppErrorMap;
  ...
  async _fillManifest(stream, uri, name) {
    ...
    icons.forEach(icon => {
      ...
      icon.src = `${uri.getBaseUri()}/tragInstance/getUveMetaData?type=${iconCode}`;
    });
      ...
      Xml2js.parseString(xmlFromString, (err, data) => {
        ...
        keys.forEach(key => {
          ...
          if (tile) {
            ...
            tile.src = `${uri.getBaseUri()}/tragInstance/getUveMetaData?type=${iconCode}`;
          }
        });
  ...
  async getProductInfo(awid) {
    ...
    // hds 2
    return {
      name: tragInstance ? tragInstance.name : DEFAULTS.name,
      uuAppErrorMap: {}
    };
  }
  ...
  async getProductLogo(awid, dtoIn) {
    ...
    let dtoOut = {};
    let tragInstance = await this.dao.getByAwid(awid);
    if (tragInstance && tragInstance.logos && tragInstance.logos.includes(type)) {
      try {
        dtoOut = await UuBinaryAbl.getBinaryData(awid, { code: type });
      } catch (e) {
        // A3
        if (logger.isWarnLoggable()) {
          logger.warn(`Unable to load uuBinary logo ${type} for trag instance ${awid}. Error: ${e} `);
        }
  ...
  async getUveMetaData(awid, dtoIn) {
    // hds 1
    let validationResult = this.validator.validate("tragInstanceGetUveMetaDataDtoInType", dtoIn);
    // A1, A2
    let uuAppErrorMap = ValidationHelper.processValidationResult(
      dtoIn,
      validationResult,
    ...
    if (!uveMetaData) {
      tragInstance = await this.dao.getByAwid(awid);
      uveMetaData = tragInstance.uveMetaData ? tragInstance.uveMetaData : {};
      this.metaDataCache.set(awid, uveMetaData);
    }
    ...
    if (uveMetaData && uveMetaData[dtoIn.type]) {
      try {
        ...
        // A3
        if (logger.isWarnLoggable()) {
          logger.warn(`Unable to load uuBinary meta data ${dtoIn.type} for trag instance ${awid}. Error: ${e} `);
        }
  ...
  async getIndex(awid, uri) {
    ...
    if (!uveMetaData) {
      let tragInstance = await this.dao.getByAwid(awid);
      uveMetaData = tragInstance && tragInstance.uveMetaData ? tragInstance.uveMetaData : {};
      uveMetaData.name = tragInstance ? tragInstance.name : DEFAULTS.name;
      uveMetaData.description =
        tragInstance && tragInstance.description ? tragInstance.description : DEFAULTS.description;
      this.metaDataCache.set(awid, uveMetaData);
    }
    ...
    <link rel="icon" href="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=favicon-32x32"/>
    <link rel="icon" type="image/png" sizes="16x16" href="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=favicon-16x16"/>
    <link rel="icon" type="image/png" sizes="32x32" href="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=favicon-32x32"/>
    <link rel="icon" type="image/png" sizes="96x96" href="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=favicon-96x96"/>
    <link rel="icon" type="image/png" sizes="194x194" href="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=favicon-194x194"/>

    <meta name="mobile-web-app-capable" content="yes">
    <meta name="apple-mobile-web-app-title" content="${uveMetaData.name}">
    <meta name="apple-mobile-web-app-status-bar-style" content="default">
    <link rel="manifest" href="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=manifest"/>

    <link rel="apple-touch-icon-precomposed" sizes="57x57" href="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=touchicon-57x57"/>
    <link rel="apple-touch-icon-precomposed" sizes="60x60" href="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=touchicon-60x60"/>
    <link rel="apple-touch-icon-precomposed" sizes="72x72" href="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=touchicon-72x72"/>
    <link rel="apple-touch-icon-precomposed" sizes="76x76" href="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=touchicon-76x76"/>
    <link rel="apple-touch-icon-precomposed" sizes="114x114" href="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=touchicon-114x114"/>
    <link rel="apple-touch-icon-precomposed" sizes="120x120" href="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=touchicon-120x120"/>
    <link rel="apple-touch-icon-precomposed" sizes="144x144" href="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=touchicon-144x144"/>
    <link rel="apple-touch-icon-precomposed" sizes="152x152" href="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=touchicon-152x152"/>
    <link rel="apple-touch-icon-precomposed" sizes="180x180" href="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=touchicon-180x180"/>
    <link rel="apple-touch-icon-precomposed" sizes="512x512" href="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=touchicon-512x512"/>
    ...
    <meta name="msapplication-config" content="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=browserconfig"/>

    <link rel="mask-icon" href="${uri.getBaseUri()}/tragInstance/getUveMetaData?type=safari-pinned-tab" color="#d81e05"/>
    `;
  ...
  /**
   * Checks whether trag instance exists and that it is not in closed state.
   * @param {String} awid Used awid
   * @param {Error} notExistError Error thrown when trag instance does not exist
   * @param {Error} closedStateError Error thrown when trag instance is in closed state
   * @returns {Promise<{}>} trag instance
   */
  async checkInstance(awid, notExistError, closedStateError) {
    ...
    if (tragInstance.state === STATE_CLOSED) {
      throw new closedStateError(
        {},
        {
          state: tragInstance.state,
...
module.exports = new TragInstanceAbl();
